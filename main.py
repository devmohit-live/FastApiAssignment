...
from dtos.chapters_dtos import *
...
async def get_courses(
    ...
    filtered_courses = []
    if domain:
        filtered_courses = db.get_courses_by_domain(domain)
    else :
        filtered_courses = db.get_all()
...
async def get_chapter_overview(course_id: str, chapter_id: str):
    chapter = db.get_chapter_by_id(course_id, chapter_id)
    if chapter == None:
        return None
    ratings = db.get_chapter_ratings(chapter_id);
...
def rate_chapter(course_id : str, rating_req: ChapterRatingRequest):
    ...
    chapter = db.get_chapter_by_id(course_id, rating_req.chapter_id)
    print("Chapter  is ", chapter)
    if chapter == None:
        raise HTTPException(status_code=404, detail="Course not found")
    rating_req
    db.add_chapter_rating(rating_req)
...
def get_course_overview(course_id: str) -> CourseOverview:
    ...
    course = dict(course)
    ratings = db.get_course_rating(course_id)
